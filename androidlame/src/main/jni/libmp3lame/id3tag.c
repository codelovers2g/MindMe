...
static int
        id3v2_add_ucs2(lame_t gfp, uint32_t frame_id, char const *lang, unsigned short const *desc,
                       unsigned short const *text);

static int
        id3v2_add_latin1(lame_t gfp, uint32_t frame_id, char const *lang, char const *desc,
                         char const *text);
...
id3tag_set_textinfo_utf16(lame_t gfp, char const *id, unsigned short const *text) {
    ...
    if (frame_id == ID_WFED) {
        return id3v2_add_ucs2(gfp, frame_id, 0, text,
...
id3tag_set_textinfo_latin1(lame_t gfp, char const *id, char const *text) {
    ...
    if (frame_id == ID_WFED) {
        return id3v2_add_latin1(gfp, frame_id, 0, text,
                                0); /* iTunes expects WFED to be a text frame */
    }
    if (isFrameIdMatching(frame_id, FRAME_ID('T', 0, 0, 0))
        || isFrameIdMatching(frame_id, FRAME_ID('W', 0, 0, 0))) {
