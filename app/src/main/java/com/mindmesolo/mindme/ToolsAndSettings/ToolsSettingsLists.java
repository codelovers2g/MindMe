 ...
 */
public class ToolsSettingsLists extends AppCompatActivity {
    ...
    View.OnClickListener fabBtnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            try {
                startActivity(new Intent(getBaseContext(), AddLists.class));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    private void setUpUiComponents() {
        Log.i("Response", "Reload True");
        arrayList = new ArrayList<ListViewItem>();
        arrayList = dBhelper.getListNameIdLength();
    ...
    private void deleteListById(ListViewItem ListId) {
        final String listId = ListId.getId();
        final String listName = ListId.getName();
        showProgressDialog("Deleting " + listName);
        final String token = GetApiAccess();
        String org_id = dBhelper.getOrganizationId();
        RequestQueue requestQueue = Volley.newRequestQueue(getBaseContext());
        JsonObjectRequest JsonObjectRequest = new JsonObjectRequest(Request.Method.DELETE, OrganizationModel.getApiBaseUrl() + org_id + "/lists/" + listId + "",
    ...
    private void removeListConfirm(final ListViewItem ListId) {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(ToolsSettingsLists.this);
        alertDialog.setMessage("Are you sure want to remove this List?");
                            ...
                            popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                                ...
                                public boolean onMenuItemClick(MenuItem item) {
                                    ...
                                    if (id == R.id.actionUpdate) {
                                        ...
                                        Intent intent = new Intent(getBaseContext(), UpdateLists.class);
                                        intent.putExtra("ListId", contactsList.get(position).getId());
                                        intent.putExtra("ListName", contactsList.get(position).getName());
                                        startActivity(intent);
                                    }
                                    if (id == R.id.actionDelete) {
                                        popupIsVisible = false;
                                        removeListConfirm(contactsList.get(position));
