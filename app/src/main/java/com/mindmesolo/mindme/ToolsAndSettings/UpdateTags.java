...
public class UpdateTags extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ActivityTitle.setText("Update Tag");
    ...
    public void UpdateTag() {
        ...
        if (editText.getText().length() > 0 && StringUtils.isNotBlank(editText.getText().toString())) {
            ...
            String REGISTER_URL = "/mindmemobile-web/api/v1/mindmemobile/orgs/" + dBhelper.getOrganizationId() + "/tags/" + TagId;

            VolleyApi.getInstance().putJsonObject(UpdateTags.this, REGISTER_URL, getJsonStr(TagId).toString(), new VolleyApi.ApiResponse<VolleyApi.ApiResult>() {
                @Override
                public void onCompletion(VolleyApi.ApiResult result) {
                    ...
                    if (result.success && result.dataIsObject()) {
                        ...
                        try {
                            String Tagid = response.getString("id");
                            String Tagname = response.getString("name");
                            dBhelper.updateTagById(Tagid, Tagname);
                        } catch (Exception e) {
                        }
                        ToastShow.setText(UpdateTags.this, "Update Successful.", Toast.LENGTH_LONG);
                        finish();
                    } else {
            ...
            ToastShow.setText(getBaseContext(), "Please enter tag name.", Toast.LENGTH_SHORT);
        }
    }
    ...
    public String getJsonStr(String id) {
        ...
        try {
            ...
            JSONArray contactIds = new JSONArray(dBhelper.getTagContactsById(id));
